...
export function CLOCK_Table({ sequence, memorySize }) {
  const { memoryStates, referenceBitStates, pageFaults } = useMemo(() => {
    ...
    for (let i = 0; i < sequence.length; i++) {
      ...
      if (indexInMemory !== -1) {
        // Ya está en memoria: actualizar bit de referencia
  ...
  if (!sequence || sequence.length === 0 || memorySize <= 0) {
    return (
      <div className="p-6 text-center text-xl text-gray-500">
        Ingresa una secuencia y tamaño de memoria válidos para simular CLOCK.
      </div>
    );
  }
