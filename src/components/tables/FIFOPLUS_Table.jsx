...
export function FIFOPLUS_Table({ sequence, memorySize }) {
  const { memoryStates, referenceBitStates, pageFaults } = useMemo(() => {
    ...
    for (let i = 0; i < sequence.length; i++) {
      ...
      if (indexInMemory !== -1) {
        // Página ya está en memoria: activamos su bit y apagamos los demás
        referenceBits.fill(0);
      ...
      // Segundo Chance limitado (FIFO+): solo un bit puede estar en 1
      while (true) {
